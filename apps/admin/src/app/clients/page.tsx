...
export default function ClientsPage() {
  ...
  return (
    <div className="flex flex-col h-full max-w-[1600px] w-full mx-auto px-4 md:px-6 lg:px-8 pb-16 lg:pb-24">
      {/* Header Section */}
      <div className="sticky top-0 z-30 bg-background py-6 mb-6 flex flex-col lg:flex-row lg:items-center justify-between gap-6 border-b border-border-base">
        <div>
          <h1 className="text-3xl font-bold text-primary mb-2">Clients</h1>
        </div>
        ...
        <div className="flex flex-col sm:flex-row items-center gap-4 w-full lg:w-auto">
          ...
          <div className="relative w-full sm:w-48 shrink-0">
            ...
            {isFilterOpen && (
              <>
                ...
                <div className="absolute top-full left-0 right-0 mt-2 bg-surface-hover border border-border-muted rounded-2xl shadow-[0_10px_40px_rgba(0,0,0,0.5)] overflow-hidden z-40 py-2">
                  {['all', 'active', 'pending', 'on hold', 'completed'].map((status) => (
                    <button
                      ...
                      className={`w-full text-left px-5 py-2.5 hover:bg-surface-active transition-colors flex items-center gap-3 text-sm ${filterStatus === status ? 'text-primary bg-primary/5 font-medium' : 'text-text-muted'}`}
                    >
                      {status !== 'all' ? (
                        <div className={`w-2 h-2 rounded-full ${status === 'active' ? 'bg-success shadow-[0_0_8px_var(--color-success)]' : status === 'pending' ? 'bg-warning shadow-[0_0_8px_var(--color-warning)]' : status === 'completed' ? 'bg-primary' : 'bg-danger'}`}></div>
                      ) : (
          ...
          <Link href="/clients/new" className="px-6 py-3 w-full sm:w-auto bg-gradient-to-r from-primary to-primary-light text-white rounded-full font-medium shadow-[0_0_20px_rgba(0,180,216,0.3)] hover:opacity-90 transition-opacity whitespace-nowrap flex items-center justify-center gap-2 shrink-0">
          ...
          <div key={client.id} className="group relative bg-background rounded-[24px] p-[1px] shadow-2xl transition-all duration-500 hover:shadow-[0_0_40px_rgba(0,126,225,0.15)] overflow-hidden">
            ...
            <div className="relative h-full bg-surface rounded-[23px] overflow-hidden flex flex-col z-10">
              ...
              <div className="px-6 pb-6 pt-2 bg-gradient-to-b from-transparent to-surface-active/50 relative z-20">
                <div className="flex w-full bg-surface rounded-full overflow-hidden shadow-lg border-t border-border-base">
                  <a 
                    href={`tel:${client.contactNumber.replace(/[^0-9+]/g, '')}`} 
                    className="flex-1 flex items-center justify-center py-3 text-primary hover:bg-primary hover:text-white transition-colors duration-300 border-r border-border-base"
                    title="Call"
                  >
                  ...
                  <a 
                    ...
                    className="flex-1 flex items-center justify-center py-3 text-danger hover:bg-danger hover:text-white transition-colors duration-300 border-r border-border-base"
                    title="Email"
                  >
                  ...
                  <a 
                    ...
                    className="flex-1 flex items-center justify-center py-3 text-success hover:bg-success hover:text-white transition-colors duration-300"
                    title="WhatsApp"
                  >
