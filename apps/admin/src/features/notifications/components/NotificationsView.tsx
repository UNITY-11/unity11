            ...
            {(['all', 'unread'] as const).map((f) => (
              <button
                ...
                className={`px-4 py-1.5 rounded-xl text-sm font-medium transition-colors ${
                  filter === f ? 'bg-surface-hover text-foreground shadow-sm' : 'text-text-muted hover:text-foreground'
                }`}
              >
      ...
      <div className="bg-surface rounded-[24px] shadow-2xl border border-border-base overflow-hidden">
        {filteredNotifications.length > 0 ? (
          ...
          <div className="p-16 text-center">
            ...
            <p className="text-text-muted text-sm">No notifications in Sanity yet.</p>
          </div>
        )}
      </div>
    </div>
