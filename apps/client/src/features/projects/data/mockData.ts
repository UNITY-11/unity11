import { Project } from "../types";

export const mockProjects: Project[] = [
  {
    id: "ai-insights-1",
    tag1: "AI + Analytics",
    tag2: "Dashboard",
    title: "AI-Powered Insights Platform",
    description:
      "Built an insights dashboard with real-time analytics, LLM automation, and advanced monitoring tools.",
    image: "/images/blog/blog2.png",
    bg: "bg-linear-to-t from-purple-600 to-blue-300",
    date: "2024-03-15T00:00:00Z",
  },
  {
    id: "fintech-app-1",
    tag1: "Finance",
    tag2: "Mobile App",
    title: "Fintech App Modernization",
    description:
      "A complete redesign + modernization of an outdated fintech application, improving speed, UX, and scalability.",
    image: "/images/blog/blog1.png",
    bg: "bg-linear-to-tl from-gray-700 to-gray-300",
    date: "2024-02-10T00:00:00Z",
  },
  {
    id: "ecommerce-1",
    tag1: "E-commerce",
    tag2: "Branding",
    title: "Next-Gen Storefront",
    description:
      "A complete e-commerce revamp with improved conversion flow, brand identity, and blazing fast UI.",
    image: "/images/blog/blog3.png",
    bg: "bg-linear-to-t from-white to-blue-700",
    date: "2023-11-20T00:00:00Z",
  },
  {
    id: "health-tracker",
    tag1: "HealthTech",
    tag2: "Mobile App",
    title: "Vitals Health Tracker",
    description:
      "A wearable-integrated health application that syncs metrics in real-time to alert users of anomalies.",
    image: "/images/home/heroImg1.png",
    bg: "bg-linear-to-tl from-teal-700 to-teal-300",
    date: "2024-01-05T00:00:00Z",
  },
  {
    id: "saas-crm",
    tag1: "SaaS",
    tag2: "Web App",
    title: "Enterprise CRM Dashboard",
    description:
      "Scalable CRM web application built for enterprise clients with deep integrations into accounting software.",
    image: "/images/home/heroImg3.png",
    bg: "bg-linear-to-t from-indigo-900 to-indigo-400",
    date: "2023-09-12T00:00:00Z",
  },
  {
    id: "crypto-wallet",
    tag1: "Web3",
    tag2: "Blockchain",
    title: "Secure Crypto Wallet UI",
    description:
      "A non-custodial crypto wallet interface with advanced security alerts, staking pools, and portfolio tracking.",
    image: "/images/about/mission.png",
    bg: "bg-linear-to-bl from-orange-500 to-yellow-300",
    date: "2024-04-01T00:00:00Z",
  }
];
